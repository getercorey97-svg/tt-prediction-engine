...
# Bind the src directory to Python path for internal imports
...
def bootstrap_historical_records(conn, target_matches=2000):
    """
    Seeds the SQLite database up to the 2,000-match historical threshold
    if real-world ingested records have not yet reached burn-in capacity.
    """
    cursor = conn.cursor()
    ...
    # 50 simulated recurring competitive players
    ...
    for idx in range(needed):
        ...
        # Determine set dominance (sweep 3-0, moderate 3-1, narrow 3-2)
        ...
        synthetic_rows.append((
            ...
            age_diff, hand_inter, height_diff, wttr_pos_diff, wttr_points_diff,
    ...
    cursor.executemany('''
        INSERT OR IGNORE INTO matches (
            ...
            age_diff, handedness_interaction, height_diff, wttr_pos_diff, wttr_points_diff,
...
def run_seeding_and_training():
    ...
    # 1. Sequential Latent Variable Stabilization
    ...
    for _, row in matches_df.iterrows():
        ...
        # Pre-update baseline calculations
        ...
        # Combinatorial DTMC evaluation of mutual point-win advantage
        ...
        # Update latent framework incrementally
    ...
    # 3. XGBoost Sequential Rolling-Window Validation and Calibration
    print(f"[{datetime.now()}] Training XGBoost ensemble with sequential rolling validation...")
    xgb_engine = TableTennisXGBoost()
    
    # Feature columns aligned with tt_xgboost_engine.py
    feature_cols = [
    ...
    # Mark database records as processed
    cursor = conn.cursor()
