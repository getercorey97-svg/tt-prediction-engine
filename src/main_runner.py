...
def run_pipeline():
    print(f"[{datetime.now()}] Initializing Pure Mathematical Prediction Engine with Full Synthesis...")
    
    executor = PureMathExecution()
    tracker = DynamicLatentTracker(state_path=STATE_PATH)
    
    if not os.path.exists(DB_PATH):
        print(f"[{datetime.now()}] Database not found at {DB_PATH}. Aborting cycle.")
        return

    if not os.path.exists(MODEL_PATH):
        print(f"[{datetime.now()}] Calibrated model artifact not found at {MODEL_PATH}. Run seed_and_train.py first.")
        return

    # Load the calibrated model artifact directly
    calibrated_model = joblib.load(MODEL_PATH)
    print(f"[{datetime.now()}] Successfully loaded calibrated XGBoost model artifact.")

    conn = sqlite3.connect(DB_PATH)
    live_board = pd.read_sql_query("SELECT * FROM matches WHERE processed = 0", conn)
    
    if live_board.empty:
        print(f"[{datetime.now()}] No new unprocessed matches found in queue.")
        conn.close()
        return
        
    print(f"[{datetime.now()}] Synthesizing and evaluating {len(live_board)} matches...")
    ...
    for _, row in live_board.iterrows():
        ...
        # Cold start detection to prevent default rating zero-differentials
        ...
        # Determine serve and receive absorption parameters
        if is_cold_start:
        ...
        # Direct probabilistic inference using the calibrated model
        prob_win_a = float(calibrated_model.predict_proba(live_features)[0, 1])
        
        if prob_win_a >= 0.50:
            projected_winner = pA
            confidence = prob_win_a
        else:
            projected_winner = pB
            confidence = 1.0 - prob_win_a
            
        # Dispatch notification
        executor.push_android_notification(match_title, projected_winner, confidence)
        
        # Mark match as processed to prevent redundant alerts
        cursor = conn.cursor()
        cursor.execute("UPDATE matches SET processed = 1 WHERE match_id = ?", (row['match_id'],))
        
    conn.commit()
    conn.close()
    print(f"[{datetime.now()}] Evaluation and notification cycle completed successfully.")
